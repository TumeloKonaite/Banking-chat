...
from src.tests.test import TestQuestion, load_tests
...
def fetch_context(
    ...
    return retriever.retrieve(query=question, top_k=top_k, doc_type=doc_type)
...
def answer_question(
    ...
    return pipeline.answer_question(question=question, history=None, doc_type=doc_type)
...
def evaluate_retrieval(test: TestQuestion, k: int = 10) -> RetrievalEval:
    ...
    retrieved_docs = fetch_context(test.question, doc_type=test.doc_type, top_k=k)
...
def run_cli_evaluation(test_number: int):
    ...
    if test.doc_type:
        print(f"Doc Type Filter: {test.doc_type}")
